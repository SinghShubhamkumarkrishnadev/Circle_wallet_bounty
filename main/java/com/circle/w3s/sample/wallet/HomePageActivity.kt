...
class HomePageActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val buttonShowNFTs = findViewById(R.id.buttonShowNFTs)
        // Initialize RecyclerView for NFTs
        recyclerViewNFTs = findViewById(R.id.recyclerViewNFTs)
        recyclerViewNFTs.layoutManager = LinearLayoutManager(this)
        nftAdapter = NFTAdapter(getDummyNFTs()) // Provide your NFT data here
        recyclerViewNFTs.adapter = nftAdapter

        // Set up button click listener to navigate to MainActivity
        buttonShowNFTs.setOnClickListener {
    ...
    private fun getDummyNFTs():  List<NFT> {
        val nfts = mutableListOf<NFT>()
        for (i in 1..10) {
            val nft = NFT("NFT $i", "Description of NFT $i", R.drawable.ic_placeholder)
            nfts.add(nft)
        }
        return nfts
    }
